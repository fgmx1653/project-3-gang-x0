  ...
  useEffect(() => {
    async function translateLabels() {
      try {
        ...
        setAllDrinksLabel(
          await translateText("All Drinks", lang).catch(() => "All Drinks")
        );
        setMilkTeaLabel(
          await translateText("Milk Tea", lang).catch(() => "Milk Tea")
        );
        setGreenTeaLabel(
          await translateText("Green Tea", lang).catch(() => "Green Tea")
        );
        setBlackTeaLabel(
          await translateText("Black Tea", lang).catch(() => "Black Tea")
        );
        setSeasonalLabel(
        ...
        setSubtotalLabel(
          await translateText("Subtotal", lang).catch(() => "Subtotal")
        );
        setTotalLabel(await translateText("Total", lang).catch(() => "Total"));
        setViewCartLabel(
          await translateText("View Cart", lang).catch(() => "View Cart")
        );
        setYourCartLabel(
          await translateText("Your Cart", lang).catch(() => "Your Cart")
        );
        setRemoveLabel(
                ...
                <div className="grid grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-6 pb-6">
                  {translatedMenuItems
                    ...
                    .map((item, idx) => (
                      <Card
                        key={item.id}
                        onClick={() => addToCart(menuItems[idx])}
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
          ...
          <div className="relative bg-white w-full max-w-lg h-[85vh] sm:h-[80vh] sm:rounded-2xl shadow-2xl flex flex-col animate-in slide-in-from-bottom-10">
            <div className="flex items-center justify-between p-6 border-b">
              <h2 className="text-2xl font-bold font-deco">{yourCartLabel}</h2>
