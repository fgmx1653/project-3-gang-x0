...
export async function PUT(req: Request) {
  try {
    ...
    // map incoming JS field names directly to DB columns (expects: name, price, isavail, seasonal, isexclusive)
    // convert boolean fields to integers for DB
    const setClauses = keys.map((k, i) => `${k} = $${i + 1}`).join(", ");
    const values = keys.map((k) => {
      const val = (fields as any)[k];
      // convert booleans to 0/1 for isavail, seasonal and isexclusive
      if (
        (k === "isavail" || k === "seasonal" || k === "isexclusive") &&
        typeof val === "boolean"
      ) {
        return val ? 1 : 0;
      }
...
export async function DELETE(req: Request) {
  try {
    ...
    await pool.query("DELETE FROM menu_recipe WHERE menu_item_id = $1", [id]);
