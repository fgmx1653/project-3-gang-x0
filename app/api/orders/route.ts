...
export async function POST(req: Request) {
  ...
  try {
    const { items, employeeId } = await req.json();
    ...
    const maxOrderIdResult = await client.query('SELECT COALESCE(MAX(order_id), 0) + 1 as next_id FROM orders');
    ...
    await client.query(
      ...
      [
        ...
        'profit',
        `Order ID: ${nextOrderId}, Revenue: $${totalRevenue.toFixed(2)}, Cost: $${totalCost.toFixed(2)}, Profit: $${profit.toFixed(2)}, Date: ${date} ${time}`
      ]
    );
